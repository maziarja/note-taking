...
function SansSerif() {
  ...
  return (
    <div
      onClick={() => {
        setFontState("sans-serif");
      }}
      className={`${fontState === "sans-serif" ? "bg-neutral-100" : "bg-neutral-0"} flex cursor-pointer items-center gap-4 rounded-xl border border-neutral-200 p-4`}
    >
      ...
      <div className="flex flex-col gap-[6px]">
        <p className="text-preset-4 text-neutral-950">Sans-serif</p>
        <p className="text-preset-6 text-neutral-700">
          Clean and modern, easy to read.
        </p>
      </div>
      <div
        className={`ml-auto h-4 w-4 rounded-full ${fontState === "sans-serif" ? "border-4" : "border-2"} ${fontState === "sans-serif" ? "border-blue-500" : "border-neutral-300"}`}
      ></div>
    </div>
  );
}
