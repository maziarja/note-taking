...
function SystemMode() {
  ...
  return (
    <div
      onClick={() => setColorState("system")}
      className={`${colorState === "system" ? "bg-neutral-100" : "bg-neutral-0"} flex cursor-pointer items-center gap-4 rounded-xl border border-neutral-200 p-4`}
    >
      ...
      <div
        className={`ml-auto h-4 w-4 rounded-full ${colorState === "system" ? "border-4" : "border-2"} ${colorState === "system" ? "border-blue-500" : "border-neutral-300"}`}
      ></div>
    </div>
  );
}
