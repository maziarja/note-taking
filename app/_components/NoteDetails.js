...
import { formattedDate, getNoteById } from "../_lib/data-service";
import NoteContent from "./NoteContent";
import NoteTags from "./NoteTags";
import NoteTitle from "./NoteTitle";

function NoteDetails({ selectedId }) {
  const [note, setNote] = useState(null);

  useEffect(() => {
    async function getNote() {
      const note = await getNoteById(selectedId);
      setNote(note);
    }
    getNote();
  }, [selectedId]);
  if (note === null) return;
  return (
    <div className="lg:border-r lg:border-neutral-200 lg:px-6 lg:py-5">
      ...
      <div className="flex flex-col gap-3 border-b border-neutral-200 py-4">
        ...
        <div className="grid grid-cols-[1fr_2fr] items-center sm:grid-cols-[1fr_4fr] lg:grid-cols-[1fr_2fr]">
          <div className="flex items-center gap-[6px]">
            <svg
              width="16"
              height="16"
