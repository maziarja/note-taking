...
function MonoSpace() {
  const { fontState, setFontState } = useFont();
  return (
    <div
      onClick={() => setFontState("monospace")}
      className={`${fontState === "monospace" ? "bg-neutral-100" : "bg-neutral-0"} flex cursor-pointer items-center gap-4 rounded-xl border border-neutral-200 p-4`}
    >
      ...
      <div className="flex flex-col gap-[6px]">
        <p className="text-preset-4 text-neutral-950">Monospace</p>
        <p className="text-preset-6 text-neutral-700">
          Code-like, great for a technical vibe.
        </p>
      </div>
      <div
        className={`ml-auto h-4 w-4 rounded-full ${fontState === "monospace" ? "border-4" : "border-2"} ${fontState === "monospace" ? "border-blue-500" : "border-neutral-300"}`}
      ></div>
    </div>
  );
}
