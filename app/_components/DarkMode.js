...
function DarkMode() {
  ...
  return (
    <div
      onClick={() => setColorState("dark")}
      className={`${colorState === "dark" ? "bg-neutral-100" : "bg-neutral-0"} flex cursor-pointer items-center gap-4 rounded-xl border border-neutral-200 p-4`}
    >
      ...
      <div
        className={`ml-auto h-4 w-4 rounded-full ${colorState === "dark" ? "border-4" : "border-2"} ${colorState === "dark" ? "border-blue-500" : "border-neutral-300"}`}
      ></div>
    </div>
  );
}
